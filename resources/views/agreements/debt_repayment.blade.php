...
<li>
    Согласно @if ($data->basis_for_issuing_loan == 'receipt')
        расписке <b>от {{ formatDate($data->receipt_date, 'd.m.Y') }}г</b>,
    ...
    Стороной-2 составляет <b>{{number_format((int) str_replace(' ', '', $data->amount_of_debt), 0, '.', ' ')}}
...
порядке: <b>каждое {{ formatDate($data->repayment_start_date, 'd') }} число месяца начиная с
    ...
    реквизиту: ИИК: {{ $data->iik }}, БИК: {{ $data->bik }} в {{getBankName($data->bank_id, 2)}}</b>
принадлежащий Стороне-2.</li>
