        ...
        @foreach($parties as $key => $party)
            <p style="text-indent: 22px">
                @if ($party->is_mediator === 0)
                    ...
                    {{ $party->data->location->street }} д. {{ $party->data->location->house }} @if(isset($party->data->location->flat))кв. {{ $party->data->location->flat }}@endif</b>, именуемый(-ая) в дальнейшем <b>"Сторона@if(count($parties) > 2)-{{$key + 1}}@endif"</b> @if($key > 0) c другой стороны, @if($key === (count($parties) - 2))совместно именуемые <b>"Стороны"</b>@else и @endif @else с одной стороны и @endif<br>
                @else
                    @if($doctype === 'contract')
